...
public class Pickaxe : Tile
{
	...
	void Update()
	{
		...
		if (_tileHoldingUs != null)
		{
			tileName = string.Format("Pickaxe (HP: {0})", health);
		}
	}
	...
	void OnTriggerEnter2D(Collider2D other)
	{
		...
		{
			Wall otherTile = other.gameObject.GetComponent<Wall>();
			if (otherTile != _tileHoldingUs && otherTile.hasTag(TileTags.Wall))
			{
				otherTile.takeDamage(this, 100, DamageType.Explosive);
				takeDamage(this, 1);
			}
		}
		else if (_swinging && other.gameObject.GetComponent<Tile>() != null)
		{
			Tile otherTile = other.gameObject.GetComponent<Tile>();
			if (otherTile != _tileHoldingUs && (otherTile.hasTag(TileTags.Enemy)||(otherTile.hasTag(TileTags.Creature))))
			{
